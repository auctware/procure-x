...
import {
  X, Edit2, Trash2, Search, Plus,
  Building2, CheckCircle2, FileText, Filter, MoreVertical,
...
export default function CenterLimitConfiguration() {
  // Form State
  ...
  useEffect(() => {
    if (searchQuery.trim()) {
      ...
      if (query.includes('active') || query.includes('inactive')) {
        ...
        setNlpSuggestion('Searching by center name or scheme');
      }
    } else {
      setNlpSuggestion('');
    }
  ...
  // Apply Filters
  const filteredData = centerLimits.filter(item => {
    if (searchQuery.trim()) {
      const query = searchQuery.toLowerCase();
      const matchesSearch = 
  ...
  const handleEdit = (item: CenterLimit) => {
    setFormData({
  ...
  const handleDelete = (id: string) => {
    if (confirm('Are you sure you want to delete this center limit configuration?')) {
  ...
  return (
    <div className="p-8 pb-20" style={{ backgroundColor: '#FFFFFF', minHeight: '100vh' }}>
      ...
      <div className="mb-6 flex items-center gap-2" style={{ fontSize: '14px', color: '#666' }}>
        ...
        <span style={{ fontWeight: '600', color: '#222' }}>Center Limit Configuration</span>
      </div>
        ...
        <div className="px-6 py-5 border-b flex flex-col sm:flex-row sm:items-center justify-between gap-4" style={{ borderColor: '#E5EBEF' }}>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p style={{ fontSize: '13px', color: '#666', marginTop: '2px' }}>
                {filteredData.length} of {centerLimits.length} configurations
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Try: 'active configurations', 'Bangalore center'..."
                value={searchQuery}
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-16 text-center" style={{ color: '#315B78' }}>
                    ...
                    <p style={{ fontSize: '14px', color: '#666' }}>
                      {searchQuery || activeFilterCount > 0 ? 'Try adjusting your search or filters' : 'Click "Add Configuration" to create a new center limit configuration'}
                    </p>
                  </td>
                </tr>
              )}
            ...
            <div className="sticky top-0 z-10 px-8 py-6 border-b flex items-center justify-between" style={{ backgroundColor: '#FFFFFF', borderColor: '#E5EBEF' }}>
              <div>
                <h2 style={{ fontSize: '22px', fontWeight: '700', color: '#003A5D', marginBottom: '4px' }}>
                  {editingId ? 'Edit Center Limit' : 'Add Center Limit'}
                </h2>
                <p style={{ fontSize: '14px', color: '#666' }}>
                  Configure center procurement limits
                </p>
              </div>
              <button
                onClick={handleCloseDrawer}
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <CustomDropdown
                      ...
                      onChange={(value) => setFormData({ ...formData, centerName: value })}
                      options={[
                        ...
                        { value: 'Hubli South Center', label: 'Hubli South Center' }
                      ]}
