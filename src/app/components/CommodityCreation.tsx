...
import {
  X, Edit2, Trash2, Search, Package, Plus,
...
export default function CommodityCreation() {
  // Form State
  ...
  const [commodities, setCommodities] = useState<Commodity[]>([
    ...
    {
      id: '3',
      commodityCode: 'CMD003',
      commodityName: 'Pulses',
      category: 'Pulses',
      unit: 'QTL',
      description: 'Various pulses',
      mspPrice: '7000',
  ...
  useEffect(() => {
    if (searchQuery.trim()) {
      ...
      } else if (query.includes('cereals') || query.includes('pulses') || query.includes('oilseeds')) {
        setNlpSuggestion('Searching by category');
      } else if (query.includes('wheat') || query.includes('rice') || query.includes('pulses')) {
        setNlpSuggestion('Searching by commodity name');
      } else if (query.includes('qtl') || query.includes('mt') || query.includes('kg')) {
        setNlpSuggestion('Searching by unit');
      } else {
        setNlpSuggestion('Searching by commodity code or name');
      }
    } else {
      setNlpSuggestion('');
    }
  ...
  const filteredCommodities = commodities.filter(commodity => {
    if (searchQuery.trim()) {
      const query = searchQuery.toLowerCase();
      const matchesSearch = 
        commodity.commodityCode.toLowerCase().includes(query) ||
        commodity.commodityName.toLowerCase().includes(query) ||
        commodity.category.toLowerCase().includes(query);
      
      if (!matchesSearch) return false;
    }

    if (filters.status !== 'All' && commodity.status !== filters.status) return false;
    if (filters.category !== 'All' && commodity.category !== filters.category) return false;

    return true;
  });

  const generateCommodityCode = () => {
    const code = 'CMD' + Math.floor(Math.random() * 10000).toString().padStart(4, '0');
    setFormData({ ...formData, commodityCode: code });
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newCommodity: Commodity = {
      id: editingId || (commodities.length + 1).toString(),
      ...formData,
      createdDate: editingId 
        ? commodities.find(c => c.id === editingId)?.createdDate || new Date().toISOString().split('T')[0]
    ...
    if (editingId) {
      setCommodities(commodities.map(commodity => 
        commodity.id === editingId ? newCommodity : commodity
      ));
    } else {
  ...
  return (
    <div className="p-8 pb-20" style={{ backgroundColor: '#FFFFFF', minHeight: '100vh' }}>
      ...
      <div className="mb-6 flex items-center gap-2" style={{ fontSize: '14px', color: '#666' }}>
        ...
        <span style={{ fontWeight: '600', color: '#222' }}>Commodity Creation</span>
      </div>
        ...
        <div className="px-6 py-5 border-b flex flex-col sm:flex-row sm:items-center justify-between gap-4" style={{ borderColor: '#E5EBEF' }}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Try: 'active commodities', 'cereals wheat'..."
                value={searchQuery}
              ...
              {filteredCommodities.length > 0 ? (
                filteredCommodities.map((commodity) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4" style={{ color: '#315B78' }}>
                      <p style={{ fontSize: '14px', fontWeight: '600', color: '#315B78' }}>
                        ₹{commodity.mspPrice}/QTL
                      </p>
                    </td>
                    <td className="px-6 py-4" style={{ color: '#315B78' }}>
                      <p style={{ fontSize: '14px', color: '#315B78', maxWidth: '300px' }}>
                    ...
                    <td className="px-6 py-4" style={{ color: '#315B78' }}>
                      ...
                      >
                        <div
                          ...
                          style={{
                            backgroundColor: commodity.status === 'Active' ? '#00A040' : '#E94545'
                          }}
                        ></div>
                        {commodity.status}
                      </span>
                      ...
                      <div className="flex items-center justify-center">
                        <div className="relative">
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              setActiveActionMenu(activeActionMenu === commodity.id ? null : commodity.id);
                            }}
                ...
                <tr>
                  <td colSpan={7} className="px-6 py-16 text-center" style={{ color: '#315B78' }}>
                    ...
                    <p style={{ fontSize: '14px', color: '#666' }}>
                      {searchQuery || activeFilterCount > 0 ? 'Try adjusting your search or filters' : 'Click "Add Commodity" to create a new commodity'}
                    </p>
                  </td>
                </tr>
              )}
          ...
          >
            <div className="sticky top-0 z-10 px-8 py-6 border-b flex items-center justify-between" style={{ backgroundColor: '#FFFFFF', borderColor: '#E5EBEF' }}>
              <div>
                ...
                <p style={{ fontSize: '14px', color: '#666' }}>
                  {editingId ? 'Update commodity details' : 'Fill in the details to create a new commodity'}
                </p>
              </div>
              <button
                onClick={handleCloseDrawer}
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="mb-8">
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label style={{ fontSize: '12px', fontWeight: '700', color: '#777777', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: '8px', display: 'block' }}>
                          Commodity Name <span style={{ color: '#E94545' }}>*</span>
                        </label>
                        <input
                          type="text"
                          required
                      ...
                      <div>
                        <label style={{ fontSize: '12px', fontWeight: '700', color: '#777777', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: '8px', display: 'block' }}>
                          Commodity Code <span style={{ color: '#E94545' }}>*</span>
                        </label>
                        <input
                          type="text"
                          required
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        <label style={{ fontSize: '12px', fontWeight: '700', color: '#777777', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: '8px', display: 'block' }}>
                          Unit <span style={{ color: '#E94545' }}>*</span>
                        </label>
                        <CustomDropdown
                          value={formData.unit}
                          ...
                          onToggle={() => setUnitDropdownOpen(!unitDropdownOpen)}
                          dropdownRef={unitRef}
                        />
                      </div>
                      ...
                      <div>
                        <label style={{ fontSize: '12px', fontWeight: '700', color: '#777777', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: '8px', display: 'block' }}>
                          MSP Price (₹/QTL) <span style={{ color: '#E94545' }}>*</span>
                        </label>
                        <input
                          type="number"
                          required
                ...
                <div className="flex items-center gap-3 pt-6 border-t sticky bottom-0 bg-white pb-6" style={{ borderColor: '#E5EBEF' }}>
                  ...
                  >
                    ...
                    {editingId ? 'Update Commodity' : 'Create Commodity'}
                  </button>
