...
import {
  ...
  MapPin, CheckCircle2, FileText, Filter, MoreVertical,
...
export default function StateLimitConfiguration() {
  // Form State
  ...
  useEffect(() => {
    if (searchQuery.trim()) {
      ...
      if (query.includes('active') || query.includes('inactive')) {
        ...
        setNlpSuggestion('Searching by scheme');
      }
    } else {
      setNlpSuggestion('');
    }
  ...
  const handleEdit = (item: StateLimit) => {
    setFormData({
      state: item.state,
      agency: item.agency,
      scheme: item.scheme,
  ...
  const handleDelete = (id: string) => {
    if (confirm('Are you sure you want to delete this state limit configuration?')) {
  ...
  return (
    <div className="p-8 pb-20" style={{ backgroundColor: '#FFFFFF', minHeight: '100vh' }}>
      ...
      <div className="mb-6 flex items-center gap-2" style={{ fontSize: '14px', color: '#666' }}>
        ...
        <span style={{ fontWeight: '600', color: '#222' }}>State Limit Configuration</span>
      </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p style={{ fontSize: '13px', color: '#666', marginTop: '2px' }}>
                {filteredData.length} of {stateLimits.length} configurations
              </p>
            </div>
          </div>
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-16 text-center" style={{ color: '#315B78' }}>
                    ...
                    <p style={{ fontSize: '14px', color: '#666' }}>
                      {searchQuery || activeFilterCount > 0 ? 'Try adjusting your search or filters' : 'Click "Add Configuration" to create a new state limit configuration'}
                    </p>
                  </td>
                </tr>
              )}
            ...
            <div className="sticky top-0 z-10 px-8 py-6 border-b flex items-center justify-between" style={{ backgroundColor: '#FFFFFF', borderColor: '#E5EBEF' }}>
              <div>
                <h2 style={{ fontSize: '22px', fontWeight: '700', color: '#003A5D', marginBottom: '4px' }}>
                  {editingId ? 'Edit State Limit' : 'Add State Limit'}
                </h2>
                <p style={{ fontSize: '14px', color: '#666' }}>
                  Configure state procurement limits
                </p>
              </div>
              <button
                onClick={handleCloseDrawer}
